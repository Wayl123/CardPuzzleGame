...
func _create_menu() -> void:
	...
	pressedStyleBox.bg_color = Color(0.75, 0.75, 0.75, 0.5)
	
	for infoSection in gameInfoData:
		var sectionButton = Button.new()
		sectionButton.name = infoSection
		sectionButton.text = infoSection
		sectionButton["theme_override_styles/pressed"] = pressedStyleBox
		sectionButton["theme_override_styles/focus"] = StyleBoxEmpty.new()
		sectionButton.toggle_mode = true
...
func _update_content(pText : String) -> void:
	for menuButtons in menuList.get_children():
		if not menuButtons.has_focus():
			menuButtons.button_pressed = false
	content.text = pText
